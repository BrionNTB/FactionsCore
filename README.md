...
| Economy commands | `plugins/FactionsCore/.../economy/` — `/balance` (`/bal`, `/money`), `/pay`, `/baltop`, all backed by the same balance ledger `/sell` deposits into. |
| `/wild` (`/rtp`) | `plugins/FactionsCore/.../misc/WildCommand.java` — random wilderness teleport inside the border, skipping claims/liquids/ungenerated chunks, with a cooldown. |
| Kits | `plugins/FactionsCore/.../kit/` — config-defined kits (`kits:` in config.yml) with per-player cooldowns persisted in SQLite and optional permission gates. `/kit`. |
| Bounties | `plugins/FactionsCore/.../bounty/` — `/bounty set <player> <amount>` escrows the money out of the placer's balance immediately; stacking bounties adds to the pot; the killer is paid and it's broadcast. |
| Faction chat + chat prefixes | `/f chat` toggles faction-only chat (implemented via `PlayerChatEvent` recipients, so other chat plugins still see a normal event); public messages get a `[FactionName]` prefix. |
| KOTH | `plugins/FactionsCore/.../koth/` — `/koth set <radius>` defines the hill, `/koth start` opens the event; first player to hold it uncontested (no other faction inside) for `koth.capture-seconds` wins money plus configurable reward commands. |
| Envoys (supply drops) | `plugins/FactionsCore/.../envoy/` — `/envoy setcenter <radius>` then `/envoy start` (or an auto-interval) scatters loot chests over the warzone with coordinates broadcast to everyone; chests despawn after a timeout. The signature SaicoPvP-style event. |
| Crates + keys | `plugins/FactionsCore/.../crate/` — bind any chest to a tier with `/crate set`, give glinting key items (`factionscore:key_common/rare/legendary`) with `/crate give`; redeeming a key rolls one weighted reward from the tier's config loot table (items or console commands, so crates can pay out enchant books, sell wands, spawners, more keys...). |
| Gen buckets | `plugins/FactionsCore/.../genbucket/` — `factionscore:genbucket_cobblestone/obsidian/sand`; click a block face and a line of blocks generates from it over time (vertical column on top/bottom faces, horizontal run on side faces), stopping at obstructions, claim borders, or max length. The standard factions wall-building tool. |
| Sell wands | `factionscore:sell_wand` (`/sellwand <player> [uses]`) — right-click a chest to sell its entire sellable contents at current /sell prices; limited uses tracked in the item's NBT. |
| TNT-damageable obsidian | `plugins/FactionsCore/.../raid/ObsidianBreakListener.java` — N TNT explosions near an obsidian block break it (`raid.obsidian-breaker` in config), restoring the raid loop that blast-proof obsidian otherwise kills. Hit counters are in-memory only; a restart resets partially-damaged blocks, which is the standard tradeoff for this feature. |
