...
public class BlockEntityMobSpawner extends BlockEntitySpawnable {
    ...
    /** Mobs spawned per cycle (see {@link #onUpdate()}); exposed so spawner-stacking plugins can scale it. */
    public void setSpawnCount(int minCount, int maxCount) {
        if (minCount > maxCount || minCount < 1) {
            return;
        }
        this.minSpawnCount = minCount;
        this.maxSpawnCount = maxCount;
        saveNBT();
    }
}
