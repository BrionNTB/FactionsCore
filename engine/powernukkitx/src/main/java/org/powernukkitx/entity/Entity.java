...
public abstract class Entity extends Location implements Metadatable, EntityID {
    ...
    /**
     * Entities used for cannoning (primed TNT, TNT minecarts, falling blocks used as cannon
     * ammo) need their position/motion mirrored to clients every tick with no dead-zone:
     * factions cannon timing is read visually off the entity's position by the player, so any
     * gating here directly desyncs cannon math from what the server actually simulates.
     */
    ...
    public void updateMovement() {
        ...
        boolean precise = requiresPreciseMovementSync();
        double positionEpsilon = precise ? 0.0d : 0.0001;
        double motionEpsilon = precise ? 0.0d : 0.0025;
        double motionRestEpsilon = precise ? 0.0d : 0.0001;

        if (diffPosition > positionEpsilon || diffRotation > 1.0) { //0.2 ** 2, 1.5 ** 2
        ...
        if (diffMotion > motionEpsilon || (diffMotion > motionRestEpsilon && this.getMotion().lengthSquared() <= 0.0001)) { //0.05 ** 2
